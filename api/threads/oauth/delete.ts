...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (userId) {
      ...
      console.log(`Threads data deletion requested for user: ${userId}`)

      // 回傳確認 URL 和 confirmation code（Meta 要求的格式）
      const confirmationCode = `delete_${userId}_${Date.now()}`

      return res.status(200).json({
        url: `https://content-rewriter.vercel.app/data-deletion-status?id=${confirmationCode}`,
